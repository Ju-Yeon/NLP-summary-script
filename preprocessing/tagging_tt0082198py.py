...
person_re = re.compile(' {25}')                   # 등장인물 2
line_re = re.compile(' {11}')                     # 대사 3
not_command_re = re.compile('[a-z]+')             # 지시 4

is_contd_re = re.compile('CONT')
is_os_re = re.compile('(O.S.)')
is_vo_re = re.compile('(V.O)')

in_re = re.compile('INT. ')                         # 장소(내부/외부) 5
...
for sc in script:
    if sentence_re.search(sc):
        if person_re.search(sc):
            ...
            if not not_command_re.search(sc):
                if is_contd_re.search(sc) or is_os_re.search(sc) or is_vo_re:
                    df.loc[sentence_cnt, 'sentence'] = sc.strip()
                    df.loc[sentence_cnt, 'label'] = 2
                    sentence_cnt += 1
                else:
                    df.loc[sentence_cnt, 'sentence'] = sc.strip()
                    df.loc[sentence_cnt, 'label'] = 12
                    sentence_cnt += 1
            else:
                if sc.rfind(')') != -1:
